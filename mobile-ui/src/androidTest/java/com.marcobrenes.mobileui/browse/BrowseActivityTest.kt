...
import com.marcobrenes.mobileui.test.factory.TestProjectFactory
...
class BrowseActivityTest {
    ...
    @get:Rule val activity = ActivityTestRule<BrowseActivity>(
    ...
    @Test fun activityLaunches() {
        stubProjectsRepositoryGetProjects(
                Observable.just(
                        listOf(TestProjectFactory.makeProject()
                        )
                )
        )
    ...
    @Test fun projectsDisplay() {
        val projects = TestProjectFactory.makeProjectList(10)
